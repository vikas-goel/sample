...
func main() {
	...
	candidateBoards(&allBoards, initBoard, low, high, &total, &valid)

	fmt.Println("Valid boards:", valid, "of", total)
	fmt.Println("Execution time:", time.Since(start))
}

func candidateBoards(all *[][]int, board []int, start, end int, total, valid *int) {
	if start == end {
		//newBoard := make([]int, len(board))
		//copy(newBoard, board)
		//*all = append(*all, newBoard)
		*total++
		...
		for i := start; i <= end; i++ {
			...
			candidateBoards(all, board, start+1, end, total, valid)
			swapPositions(board, start, i)
		}
	}
}
